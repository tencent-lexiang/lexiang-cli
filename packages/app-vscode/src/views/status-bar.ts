...
/**
 * 状态栏按钮，显示当前已激活的知识库数量和后台任务进度。
 *
 * - 无知识库：显示 "$(database) 乐享"
 * - 有知识库：显示 "$(database) 乐享 N 个运行中"
 * - 有后台任务：显示 "$(sync~spin) 乐享 同步中 3/10 文档名..."
 * - 点击后弹出 QuickPick，允许选择停用某个知识库
 */
export class StatusBarItem {
  ...
  constructor(private readonly spaceRegistry: SpaceRegistry) {
  ...
  private render(): void {
    const services = this.spaceRegistry.getAll();
